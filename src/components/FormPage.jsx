...
const FormPage = ({setDashboard,setFinalData,formData,setFormData,toBeUpdated}) => {
  ...
  return (
     <section>
        <form className='flex flex-col gap-5 border border-gray-600 p-6 rounded-2xl' onSubmit={handleSubmit}>
          ...
          <button className='bg-black text-white border border-gray-500 py-4 px-10 cursor-pointer' type='submit'>{toBeUpdated ? 'Update': 'Sub'}</button>
        </form>
      </section>
  )
}
